...
namespace detail
{
	...
	struct compute_fract<L, T, Q, true>
	{
		...
		{
			vec<L, T, Q> result;
			result.data = v.data - __builtin_elementwise_floor(v.data);
			return result;
		}
	};
	...
	//
	// NOTE: the elementwise min/max/clamp stuff does not follow what GLM's
	// test suite expects for handling NaNs.
	//
